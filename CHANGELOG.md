...
- `sample-tvos/`: a runnable Apple TV sample with the Xcode project to host it, showing the library in all three layouts (the `row { }` DSL column, the state-based column, a standalone DSL `RokuLazyRow` with a hoisted state — Play/Pause on the remote cycles them) driven by one touchpad navigator. Siri Remote touchpad movement is read by a `UIPanGestureRecognizer` and paced the way the native focus engine paces it — the highlight follows the thumb, a fast thumb covers more ground, nothing moves once it lifts, and sub-step travel leans the focused card toward the thumb (the focus-movement hint) so a small swipe is never mistaken for a lost one. An on-screen readout shows the last gesture's distance, step count and peak speed, and the frame timing it produced against the panel's real refresh rate. The library itself stays input-agnostic; every gesture line lives in the sample.

- 41 new tests: `RokuMoveByTest` (bounds, wrap-around, single-step parity, the coalescing assertion by *counting* `onItemSelected`, edge policy once, partly-consumable moves, the key-repeat arbiter on both paths, and `moveItemsBy` / `rokuMoveItemsBy` through the column's active row) and `RokuSwipeVelocityTest` (threshold, growth, cap, sign, monotonic sensitivity, override, degenerate configs, positional compatibility).
