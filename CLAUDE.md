...
| `RokuTouchpadConfig.kt` | Public data class: `itemStepFraction` 0.65, `rowStepFraction` 1, `axisLock` 16, gain 2,500 → 12,000 units/s to ×2, `hintTravel` 14 dp, `hintTiltDegrees` 6, `hintScale` 1.04, `hintHighlightParallax` 1.35, `hintReleaseSpec` spring(0.55, 450). `gain(speed)` is the smoothstep. |
| `RokuTouchBinding.kt` | Internal glue: `RokuTouchTarget` (`stepPx(orientation)`, `moveItems`, `moveRows` → moved?), `BindRokuTouchpad` (bind while focused), `rememberRokuTouchLean` (an `Animatable<Offset>` that `snapTo`s while dragging and `animateTo(Zero, hintReleaseSpec)` on release; null without a touchpad), `RokuTouchLeanStyle`, `GraphicsLayerScope.applyTouchLean` (sqrt-shaped translation + tilt + lift, additive so the overlays apply it after positioning), `Modifier.rokuTouchLean`, `Modifier.rokuTouchpadKeyGuard`. |
...
| `RokuFocusConfig.kt` | Config data class: animation spec, key repeat delay + acceleration, wrapAround, haptics, `focusEscape`. Touchpad pacing is on `RokuTouchpadConfig`, not here (an earlier unreleased draft had fling-to-steps knobs and `stepsForVelocity` on this class; the drag model replaced them before 2.3.0 shipped). Keeps a deprecated `allowFocusEscape` factory and read-path extension. |
...
`RokuMoveByTest`, `RokuTouchpadTest`, `RokuGridStateTest`.
...
Wrap-Around, Floating Focus, Grid — a 5-column `RokuFocusGrid` of portrait cards filling their
cells — and Plain Compose comparison). Images from `picsum.photos`. Screens are wrapped in a
